import { StaticImageData } from "next/image";
import Landscaping from "../../public/assets/landscape.png";
import PruningPlants from "../../public/assets/project1.png";

type ServiceDetails = {
  id: string;
  title: string;
  body: string;
  img: StaticImageData;
};

export const services = [
  {
    title: "Landscaping",
    href: "/Services/Landscaping",
  },
  {
    title: "Pruning Plants",
    href: "/Services/Pruning-Plants",
  },
  {
    title: "Irrigation & Drainage",
    href: "/Services/Irrigation-and-Drainage",
  },
  {
    title: "Garden Maintenance",
    href: "/Services/Garden-Maintenance",
  },
];

export const details: ServiceDetails[] = [
  {
    id: "Landscaping",
    title:
      "Transform Your Outdoor Space with Our Professional Landscaping Services",
    body: `Our expert landscaping services are designed to transform your outdoor spaces into breathtaking environments that blend beauty and functionality. Whether you're looking to create a serene garden retreat, a vibrant flower bed, or a sustainable, low-maintenance landscape, we tailor our services to meet your unique vision.
  
  We specialize in designing and maintaining lush lawns, installing hardscapes like patios and walkways, and integrating water features that enhance the natural beauty of your property. Our team of skilled landscapers uses top-quality materials and the latest techniques to ensure that your outdoor space not only looks stunning but also thrives year-round.
  
  From seasonal planting to comprehensive lawn care, our landscaping services are customized to suit the specific needs of your property. We take pride in our attention to detail and our commitment to creating outdoor spaces that reflect your personal style and add value to your home.
  
  Trust us to turn your landscaping dreams into reality, with designs that are both aesthetically pleasing and environmentally sustainable. Let us help you create an outdoor space where you can relax, entertain, and enjoy the beauty of nature right in your own backyard.`,
    img: Landscaping,
  },
  {
    id: "Pruning-Plants",
    title: "Expert Pruning Services for Healthy and Beautiful Plants",
    body: `Maintaining the health and aesthetics of your plants is essential for a vibrant garden. Our professional pruning services ensure that your trees, shrubs, and flowering plants are pruned with precision and care. 
  
  Pruning not only enhances the appearance of your plants but also promotes healthy growth by removing dead or diseased branches, improving air circulation, and allowing sunlight to penetrate the foliage. 
  
  Whether it's shaping ornamental trees, trimming overgrown hedges, or deadheading flowers, our team of experts uses the latest techniques to keep your garden looking its best. 
  
  Regular pruning prevents potential hazards from weak branches and encourages the development of strong, healthy structures. With our pruning services, you can enjoy a lush, well-maintained landscape that enhances your outdoor space year-round. Trust us to provide the care your plants deserve, ensuring they thrive and contribute to the overall beauty of your garden. 
  
  Contact us today to schedule your pruning service and give your garden the professional touch it needs to flourish.`,
    img: PruningPlants,
  },
  {
    id: "Irrigation-and-Drainage",
    title: "Irrigation & Drainage",
    body: `Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. At vero eos et accusam et justo duo dolores et ea

Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet. Lorem

`,
    img: Landscaping,
  },
  {
    id: "Garden-Maintenance",
    title: "Garden Maintenance",
    body: `Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. At vero eos et accusam et justo duo dolores et ea

Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet. Lorem

`,
    img: Landscaping,
  },
];
